...
const about = (props) => {
  ...
  const [SkillOption, setSkillOption] = useState(["Maths", "Science"]);
      ...
      return Result.map((x) => (
        <Cards
          name={
            ...
            " " +
            Math.round(
              Number(Math.max(Number(x[0]) - Number(x[1]), 0)) * 100
            ).toString() +
            " -> " +
            Math.round(
              Number(Math.min(Number(x[0]) + Number(x[1]), 1)) * 100
            ).toString() +
            "%"
          }
          to={`${config.Path}view?uni=${String(x[2]).split("/")[0]}&fac=${
            String(x[2]).split("/")[1]
          }`}
        />
      ));
  ...
  const generateParameters = () => {
    ...
    var factor = 1;
    SkillArr.forEach((x) => {
      parameter[0][SkillOption.indexOf(x)] += factor;
      factor /= 1.2;
    });
    factor = 1;
    InterestArr.forEach((x) => {
      parameter[1][InterestOption.indexOf(x)] += factor;
      factor /= 1.2;
    });
  ...
  const Dropdown = (props) => {
    ...
    return (
      <Select
        variant="outlined"
        value={value}
        className="truncate w-full max-w-200 rounded-2xl bg-white h-10 text-lg text-center content-center"
        onChange={(e) => {
          console.log(e);
          if (typeof e !== "undefined") setstate(e);
        }}
      >
        {option.map((a) => (
          <Option value={a}>{textmapper(a)}</Option>
        ))}
      </Select>
    );
  };
              ...
              <div className="relative h-full pt-12 pb-3.5 pl-10 w-full bg-white rounded-xl max-md:mt-10">
                <div className="flex flex-col p-2 gap-2 w-6/12 max-md:gap-0 max-md:ml-0 max-md:w-full">
                  <div className="flex flex-col p-2 gap-2 w-6/12 max-md:gap-0 max-md:ml-0 max-md:w-full ">
                    ...
                    {[
                      ...Array(
                        Math.min(
                          SkillOption.length,
                          Selection.Sel[0].length + 1
                        )
                      ).keys(),
                    ].map((i) => (
                      <>
                        <Dropdown
                          option={SkillOption.filter(
                            (element) =>
                              !Selection.Sel[0].includes(element) ||
                              Selection.Sel[0][i] == element
                          ).concat(["Unselect"])}
                          value={Selection.Sel[0][i]}
                          setstate={(x) => {
                            setSelection((a) => {
                              setDummyState((x) => !x);
                              if (x === "Unselect") {
                    ...
                    {[
                      ...Array(
                        Math.min(
                          ...
                          Selection.Sel[1].length + 1
                        )
                      ).keys(),
                    ].map((i) => (
                      <>
                        <Dropdown
                          option={InterestOption.filter(
                            (element) =>
                              !Selection.Sel[1].includes(element) ||
                              Selection.Sel[1][i] == element
                          ).concat(["Unselect"])}
                          value={Selection.Sel[1][i]}
                          setstate={(x) => {
                            setSelection((a) => {
                              setDummyState((x) => !x);
                              if (x === "Unselect") {
                  ...
                  <div className="flex flex-col p-2 gap-2 w-6/12 max-md:gap-0 max-md:ml-0 max-md:w-full ">
                    {[
                      ...Array(
                        Math.min(
                          PersonalityOption.length,
                          Selection.Sel[2].length + 1
                        )
                      ).keys(),
                    ].map((i) => (
                      <>
                        <Dropdown
                          option={PersonalityOption.filter(
                            (element) =>
                              !Selection.Sel[2].includes(element) ||
                              Selection.Sel[2][i] == element
                          ).concat(["Unselect"])}
                          value={Selection.Sel[2][i]}
                          setstate={(x) => {
                            setSelection((a) => {
                              setDummyState((x) => !x);
                              if (x === "Unselect") {
                ...
                <div className="relative bottom-0 mb-0 p-2 flex gap-5 flex-col max-md:gap-0">
                  <div className="flex gap-2 ml-5 w-6/12 max-md:ml-0 max-md:w-full">
                    ...
                    <button
                      onClick={() => applySearch()}
                      className="flex-col grow justify-center px-4 py-3 w-full text-base text-white whitespace-nowrap rounded-2xl bg-blue max-md:px-5 max-md:mt-10 h-15"
                    >
                      Submit
                    </button>
                  </div>
